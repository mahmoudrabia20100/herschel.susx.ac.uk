...
def get_subdomains(domain, all_subdomains_file, output_folder):
    try:
        ...
        with open(all_subdomains_file, "a") as all_file, open(os.path.join(output_folder, f"{domain}_subdomains.txt"), "w") as domain_file:
            ...
            subdomains = sublist3r.main(domain, 20, None, ports=None, silent=True, verbose=False, enable_bruteforce=False, engines=None)
...
def process_domains(file_name):
    try:
        ...
        for i, domain in enumerate(domains):
            ...
            # إضافة فترة انتظار كل 100 دومين
            if (i + 1) % 100 == 0:
                print(f"Processed {i + 1} domains. Pausing for 10 seconds to prevent rate limiting...")
                time.sleep(10)  # انتظار لمدة 10 ثواني
